...
export function applySecurityRestrictions(devUrl: string) {
  ...
  const ALLOWED_ORIGINS_AND_PERMISSIONS = new Map<string, Set<Permission>>(
    CONTEXT.isDevEnvironment ? [[new URL(devUrl).origin, new Set()]]
      : [],
  );
  ...
  app.on('web-contents-created', (_, contents) => {
    ...
    contents.session.setPermissionRequestHandler((webContents, permission, callback) => {
      ...
      if (!permissionGranted && CONTEXT.isDevEnvironment) {
    ...
    contents.setWindowOpenHandler(({url}) => {
      ...
      } else if (CONTEXT.isDevEnvironment) {
